...
export function useFetch<T = any>(
  ...
  // Create a reference to the success message
  const successMessageRef = options?.successMessage;
  
  // Use the base query
  const query = useQuery<ApiResponse<T>, Error, ApiResponse<T>, QueryKey>({
    queryKey,
  ...
  React.useEffect(() => {
    if (query.isSuccess && query.data?.data && successMessageRef) {
      toast({
        title: 'Success',
        description: successMessageRef,
      });
    }
  }, [query.isSuccess, query.data, successMessageRef]);
